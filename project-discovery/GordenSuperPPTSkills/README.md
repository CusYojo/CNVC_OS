...
图片版 PPT 生成阶段默认走用户网关 `https://getways-jumu.zeelin.cn`，需设置 `GATEWAY_IMAGE_API_KEY`（或 `MODEL_GATEWAY_API_KEY` / `OPENAI_API_KEY`）；其它图像处理阶段见各技能 `references/runtime-notes.md`。
