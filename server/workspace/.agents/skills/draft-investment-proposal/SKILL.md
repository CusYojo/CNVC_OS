...
投资提案的内容质量、事实边界、章节结构、版式符合性和是否可交付，由执行本 Skill 的 Agent 依据本文件及 `references/` 完成审阅并作出结论。宿主程序不得另设业务规则、模板分数或 Word 内容扫描来推翻或重复判定 Agent 的结论，也不得因这类程序规则自动重生成文档。宿主程序只负责文件存在性、DOCX 包可读取、任务归属、存储和鉴权下载。
