...
3. 覆盖 15 个规定分类，标准版每类 1 题，深度版每类 2 题；分类仅用于内部覆盖检查，不是用户可见的固定章节。
...
6. 证据充分时，后续使用二至五个换行分隔的 `（序号）维度标题：判断。依据。影响。边界。`；维度标题必须承载判断且连续编号。
7. 一段只表达一个中心判断，同一事实不得在直接答复和分维度之间重复。
8. 所有数字带单位、期间或截止日，并能在引用来源中定位。
9. 资料不足时严格输出 `暂无相关资料。`
10. 输出结构化 JSON：`questionId/answer/sourceIndexes/supportingQuotes/confidenceStatus/missingInformation`。
...
5. 支持原文是否来自引用来源。
6. 引用是否真正支持当前回答。
7. 直接答复是否结论先行，分维度是否为二至五项且连续编号。
8. 是否区分公司陈述、预测、意向和已实现事实。
...
- Reviewer 审阅结果。
- 页眉与页码。
- 15 类只保留在内部覆盖元数据中，不写入目录，也不渲染为固定正文章节。
- 仅生成可编辑 Word 与同源 PDF，不生成 PPT/PPTX。
