...
export function projectFileAccessCondition(userId: string | SQL, operation: FileOperation = 'view', includeDeleted = false) {
  ...
  return and(
    ...
    inArray(projectFiles.projectId, db.select({ id: projects.id }).from(projects).where(projectScope)),
  )!
}
...
export function projectFileWorkspaceCondition(userId: string, operation: 'view' | 'upload' = 'view') {
  return and(currentProjectScope(userId), sql`EXISTS (SELECT 1 FROM ${users} workspace_actor WHERE workspace_actor.id=${userId} AND workspace_actor.status='启用')`, or(ne(projects.workflowModel, 'fde-v1'), and(businessRole(userId), operation === 'upload' ? eq(projects.lifecycle, 'active') : undefined)))!
}
