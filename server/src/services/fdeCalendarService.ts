...
import { personalCalendarEvents, personalCalendarHistory, todoCalendarSchedules, todoCalendarScheduleHistory, leaderTimeRequests, projectPlanActions, projectPlans, projectTimelineTasks, projects, todos, meetings, meetingParticipants, users } from '../db/schema.js'
...
export async function writeTaskCalendarSchedule(id: string, userId: string, raw: unknown) {
  ...
  return scheduleTransaction(async tx => {
    ...
    const [timelineSource] = await tx.select({ taskId: projectTimelineTasks.taskId }).from(projectTimelineTasks).where(eq(projectTimelineTasks.taskId, id))
    if (timelineSource) return timeFail('CALENDAR_TASK_FORBIDDEN', '流程时间线任务须通过项目计划或延期审批调整', 403)
...
export async function listCalendar(userId: string, rawWeek: string, view: 'personal' | 'company', includeMilestones = false) {
  ...
  return db.transaction(async (tx) => {
    ...
    const timelineTaskIds = new Set(tasks.length ? (await tx.select({ taskId: projectTimelineTasks.taskId }).from(projectTimelineTasks).where(inArray(projectTimelineTasks.taskId, tasks.map(row => row.id)))).map(row => row.taskId) : [])
    ...
    for (const row of tasks) {
      ...
      add({ id: row.id, source: 'task', title: row.title, detail: `${row.projectName ?? '个人任务'} · ${row.status}${row.dueTime ? ` · 截止 ${row.dueTime}` : ''}`, projectId: row.projectId, projectName: row.projectName ?? '个人任务', ownerId: row.ownerUserId ?? '', ownerName: row.ownerUserId ? names.get(row.ownerUserId) ?? '已停用人员' : '待绑定', startsAt: startsAt.toISOString(), endsAt: endsAt.toISOString(), allDay: false, version: timelineTaskIds.has(row.id) ? row.version : schedule?.version ?? 0, sourceVersion: row.version, editable: view === 'personal' && row.ownerUserId === userId && allowed && !timelineTaskIds.has(row.id), target: row.projectId ? `/projects/${row.projectId}?tab=tasks&task=${row.id}` : '/' }, allowed)
    }
