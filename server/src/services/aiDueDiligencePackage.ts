...
export type DueDiligencePackage = {
  reportMode?: string
...
export type NormalizedDueDiligencePackage = {
  reportMode: string
  diligenceData: Record<string, unknown>
  report: Record<string, unknown>
}

export function compactText(value: unknown, maximum = 1800) {
  return String(value ?? '')
    .replace(/\s+/g, ' ')
    .trim()
    .slice(0, maximum)
...
export function requestedReportMode(value: unknown) {
...
export function buildDueDiligencePackageModelInput(input: {
  project: ProjectLike
...
证据台账：${JSON.stringify(input.evidence).slice(0, 90_000)}`
  return { desiredMode, messages: [{ role: 'system' as const, content: systemPrompt }, { role: 'user' as const, content: userPrompt }] }
}
