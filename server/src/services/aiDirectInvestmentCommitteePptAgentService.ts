...
function directAgentRecoveryPrompt(input: {
  ...
  return `上一个 investment-committee-ppt Agent 上下文在模型网关返回普通 403 后已经关闭。当前是第 ${input.recoveryAttempt} 次全新上下文恢复，工作区中的资料、Manifest、研究底稿、页面方案、PPTX 候选稿和逐页复核文件均被原样保留。

恢复要求：
1. 首先重新使用 Skill 工具调用 investment-committee-ppt；不得沿用未重新加载 Skill 的判断。
2. 读取 ./REQUEST.json 和 ./materials/manifest.json，检查现有研究底稿和页面成果与 Manifest 的覆盖关系；若现有证据台账不能证明某个来源或片段已经纳入，必须补读缺失资料。
3. 在现有工作成果上继续，不得删除已完成的研究、页面方案、可编辑图表、候选稿或逐页复核结果后从头降级生成。
4. 页面内容、版式修订和最终验收只服从当前 Skill；宿主不会生成页面、替代审阅或提供旧模板兜底稿。
...
export async function runDirectInvestmentCommitteePptAgent(input: {
  ...
  if (input.skill.name !== 'investment-committee-ppt' || (frozen && parseAiSkillFile(Buffer.from(
    frozen.files.find(file => file.path === 'SKILL.md')!.contentBase64, 'base64').toString('utf8')).name !== input.skill.name)) {
    throw Object.assign(new Error('冻结技能包与 PPT 任务不一致'), { code: 'DIRECT_SKILL_TASK_MISMATCH' })
  }
  ...
  if (frozen) {
    ...
    if (await realpath(parent) !== path.resolve(parent)
      || !path.resolve(targetSkillDirectory).startsWith(`${path.resolve(workspace)}${path.sep}`)) throw Error('Frozen skill destination escapes task workspace')
  ...
  } else await cp(getAiSkillDirectory(input.skill.name), targetSkillDirectory, { recursive: true, force: true })
  await writeFile(
  ...
  let prompt = input.resumeExistingWorkspace
    ? directAgentRecoveryPrompt({ outputFileName, recoveryAttempt: 1 })
    : directAgentPrompt({
      project: input.project,
      sourceCutoffDate: input.sourceCutoffDate,
      instructions: input.instructions,
      ...
      try {
        await input.onProgress?.({
          stage: input.resumeExistingWorkspace || gatewayRetryCount > 0
            ? '正在启动全新 PPT Agent 上下文继续 Skill'
            : '正在启动隔离 PPT Agent 并加载 Skill',
        ...
        if (
          ...
          gatewayRetryCount += 1
          await input.onProgress?.({
            stage: `模型网关暂时拒绝，${Math.ceil(gatewayRecovery.delayMs / 1_000)}秒后由新 PPT Agent 上下文继续`,
            progress: 82,
          })
