...
committeeRouter.get('/:id/agendas/:agendaId/files/:fileId/:version/preview', async (req: AuthedRequest, res, next) => {
  try {
    ...
    res.set({ 'Content-Type': contentType, 'Content-Length': String(file.bytes.length), 'Content-Disposition': `inline; filename*=UTF-8''${encodeURIComponent(file.name)}`, 'X-Content-SHA256': file.sha256, 'X-File-Version': String(file.version), 'Content-Security-Policy': "sandbox; default-src 'none'; img-src data:" })
    res.send(file.bytes)
