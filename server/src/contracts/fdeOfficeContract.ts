...
const travel = z.object({ kind: z.literal('出差'), travelerIds: ids.default([]), startDate: date.optional(), endDate: date.optional(), destination: text.default(''), budget: officeMoney.default('0'), currency: currency.default('CNY') }).strict()
const seal = z.object({ kind: z.literal('用印'), entity: text.default(''), sealType: text.default(''), purpose: text.default(''), copies: z.number().int().min(1).max(10000).optional(), handlerId: id.optional() }).strict()
const expense = z.object({ kind: z.literal('报销'), currency: currency.default('CNY'), amount: officeMoney.default('0'), items: z.array(z.object({ id, date, category: text.min(1), description: text.min(1), amount: officeMoney, invoiceNumber: z.string().trim().max(120).default(''), attachmentId: id.optional() }).strict()).max(100).default([]) }).strict()
const leave = z.object({ kind: z.literal('请假'), leaveType: text.default(''), startAt: timestamp.optional(), endAt: timestamp.optional(), hours: z.string().regex(/^(0|[1-9]\d{0,3})(\.\d{1,2})?$/).optional() }).strict()
...
export const officeFieldNames: Record<typeof officeKinds[number], readonly string[]> = {
  出差: ['travelerIds', 'startDate', 'endDate', 'destination', 'budget', 'currency'], 用印: ['entity', 'sealType', 'purpose', 'copies', 'handlerId'],
...
export function validateOfficeSubmission(definition: OfficeDefinition, policy: OfficePolicy) {
  ...
  if (d.kind === '报销') {
    if (d.items.reduce((sum, item) => sum + officeCents(item.amount), 0n) !== officeCents(d.amount)) issues.push('报销明细与总额不一致')
