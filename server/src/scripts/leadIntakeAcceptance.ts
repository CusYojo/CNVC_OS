...
assert.doesNotMatch(service, /scheduleScoring/)
assert.match(service, /companyRegistrationEligibility\(input\.lead\.registrationStatus\)/)
assert.match(service, /status: 'rejected' as const/)
assert.match(service, /登记状态为注销，已在入池前排除/)
assert.match(storage, /mode: 0o600/)
assert.match(storage, /isSymbolicLink/)
assert.match(serverEntry, /startLeadBpWorker\(\)/)
assert.match(serverEntry, /stopLeadBpWorker\(\)/)
assert.doesNotMatch(page, /\/leads\/imports\/preview|\/leads\/bp-uploads/)
assert.doesNotMatch(page, /批量导入|上传 BP/)
...
assert.ok((rows[0] ?? []).includes('登记状态'))

console.log(JSON.stringify({ ok: true, checks: 35, templateBytes: template.length }))
