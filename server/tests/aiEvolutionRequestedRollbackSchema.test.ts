...
test('requested rollback migration binds one rollback to its verified source release', async () => {
  ...
  const index = journal.entries.findIndex((item: { tag: string }) => item.tag === '0110_add_ai_evolution_requested_rollback')
  assert.ok(index > journal.entries.findIndex((item: { tag: string }) => item.tag === '0108_add_ai_evolution_release_jobs'))
  assert.deepEqual(journal.entries[index], { idx: 111, version: '5', when: 1793173743000,
