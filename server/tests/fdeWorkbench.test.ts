...
test('dashboard heading and completion behavior are personalized without duplicating todo data', () => {
  assert.match(page, /fde-dashboard-heading:/)
  assert.match(page, /修改标题导语/)
  assert.match(page, /恢复默认/)
  assert.match(page, /fde-dashboard-todo-completion-mode:/)
  assert.match(page, /完成后隐藏/)
  assert.match(page, /保留划线/)
  assert.match(page, /status: todo\.status === '已完成' \? '未开始' : '已完成'/)
  assert.match(page, /\/todos\/personal\/completed\?before=/)
  assert.match(page, /nextShanghaiMidnight/)
  assert.match(todoRoutes, /todosRouter\.delete\('\/personal\/completed'/)
  assert.match(todoService, /archiveExpiredCompletedPersonalTodos/)
  assert.match(todoService, /lt\(todos\.dueDate, beforeDate\)/)
  assert.match(todoService, /isNull\(todos\.projectId\)/)
  assert.match(todoService, /eq\(todos\.ownerUserId, userId\)/)
  assert.match(todoService, /status: '已归档'/)
...
test('sidebar includes a low-contrast non-interactive horse silhouette', () => {
  ...
  assert.match(shell, /\.fde-sidebar-horse \{[^}]*opacity: \.5;[^}]*pointer-events: none;/)
  assert.match(shell, /\.fde-sidebar-horse img \{[^}]*object-fit: contain;[^}]*scale\(1\.04\)/)
  assert.doesNotMatch(layout, /fde-horse-(?:tail|body|mane|leg)/)
  assert.match(shell, /@media \(max-height: 720px\) \{ \.fde-sidebar-horse \{[^}]*width: 220px;/)
