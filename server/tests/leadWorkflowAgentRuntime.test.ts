...
test('routes GPT agents through the explicit Codex gateway transport', () => {
  const subject = process.env.LEAD_SUBJECT_AGENT_BACKEND
  const workflow = process.env.LEAD_WORKFLOW_AGENT_BACKEND
  try {
    process.env.LEAD_SUBJECT_AGENT_BACKEND = 'codex-gateway'
    process.env.LEAD_WORKFLOW_AGENT_BACKEND = 'codex-gateway'
    assert.equal(leadSubjectAgentRuntime('gpt-5.6-sol'), 'codex-gateway')
    assert.equal(leadWorkflowAgentRuntime('gpt-5.6-sol'), 'codex-gateway')
  } finally {
    if (subject === undefined) delete process.env.LEAD_SUBJECT_AGENT_BACKEND
    else process.env.LEAD_SUBJECT_AGENT_BACKEND = subject
    if (workflow === undefined) delete process.env.LEAD_WORKFLOW_AGENT_BACKEND
    else process.env.LEAD_WORKFLOW_AGENT_BACKEND = workflow
  }
})
...
test('audit prompt versions are isolated by Agent runtime', () => {
  ...
  assert.equal(leadAgentAuditPromptVersion('lead-research-prompt-v1', 'codex-gateway'), 'lead-research-prompt-v1-codex-gw-v1')
