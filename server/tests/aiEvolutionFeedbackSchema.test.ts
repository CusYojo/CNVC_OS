...
test('feedback migration binds one authorized subject and is registered after its dependencies', async () => {
  ...
  const index = journal.entries.findIndex((item: { tag: string }) => item.tag === '0109_add_ai_evolution_feedback')
  assert.ok(index > journal.entries.findIndex((item: { tag: string }) => item.tag === '0101_add_ai_experience_versions'))
  assert.deepEqual(journal.entries[index], { idx: 110, version: '5', when: 1793087343000,
