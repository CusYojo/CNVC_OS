...
test('正文专业化修复英文缩写、连字符技术名和 Markdown 标记', () => {
  assert.equal(
    ...
    '执行摘要：AI-Core-Tech由负责人COO推进4D产品化。',
  )
})
...
test('正文专业化修复带单位的拆分数字', () => {
  assert.equal(
    ...
    '方案以10%真实数据联动90%合成数据，规划营收4000万元、团队30人。',
  )
})

test('正文专业化清除中文与英文数字之间及中文断词空格', () => {
  assert.equal(
    professionalizeDocumentText(
      '创始人来自新 加坡，团队从事 AI 行业研发，形成 “ 真实数据 - 仿真合成 ” R2S2R 方案，营收 1,000 万元。',
    ),
    '创始人来自新加坡，团队从事AI行业研发，形成“真实数据—仿真合成”R2S2R方案，营收1,000万元。',
  )
})
