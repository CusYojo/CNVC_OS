...
export function FdeIntegrationsPanel() {
  ...
  return <div className="space-y-4">
    <div className="flex flex-wrap items-center justify-between gap-3"><p className="text-xs text-slate-500">沿用当前连接配置。刷新只读取状态，不发消息、不改密钥、不触发外部模型调用。</p><Button variant="secondary" loading={loading} onClick={() => void load()}><RefreshCw className="h-4 w-4" />刷新真实状态</Button></div>
    ...
    {summary && <>
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <Card className="fde-panel p-5"><h2 className="font-semibold">MySQL 与项目文件</h2><div className="mt-4 flex flex-wrap gap-3"><Badge tone={error ? 'amber' : 'green'}>MySQL：{error ? '当前状态未知' : '连接可达'}</Badge><Badge tone={summary.storage.accessible && !error ? 'green' : 'amber'}>项目文件目录：{error ? '当前状态未知' : summary.storage.accessible ? '可读写' : '不可访问'}</Badge></div><p className="mt-3 text-xs leading-6 text-slate-500">文件检测仅验证目录访问，不等于上传、下载或云存储往返验收。原始字节仍保存在当前文件存储。</p></Card>
        <Card className="fde-panel p-5"><h2 className="font-semibold">通知投递队列</h2><div className="mt-4 flex flex-wrap gap-2">{summary.outbox.length ? summary.outbox.map((item) => <Badge key={item.status}>{item.status}：{item.total}</Badge>) : <p className="text-sm text-slate-500">暂无投递记录</p>}</div><p className="mt-3 text-xs text-slate-500">无队列积压不等于外部渠道连接正常；连接状态见下方账号记录。</p></Card>
