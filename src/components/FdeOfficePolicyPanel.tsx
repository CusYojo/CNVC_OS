...
const blankNode = (key = 'review') => ({ key, name: '', roleIds: [] as string[], scope: 'applicant_department' as const, mode: '或签' as const, fixedUserIds: [] as string[], allowTransfer: false })
const empty = (kind: typeof officeKinds[number], roles: Option[] = [], people: { id: string; name: string; role: string }[] = []): OfficePolicy => {
  const byRole = (predicate: (role: Option) => boolean) => roles.filter(predicate).map(role => role.id)
  const supervisorRoleIds = byRole(role => role.fdeCategory === 'project_lead' || /部门负责人|投资总监|合伙人/.test(role.name))
  const leadershipRoleIds = byRole(role => role.fdeCategory === 'institution_leader' || ['董事长', '总裁'].includes(role.name))
  const leadershipUserIds = people.filter(person => ['黄昕', '陈斌'].includes(person.name) && /董事长|总裁/.test(person.role)).map(person => person.id)
  if (kind === '出差') return {
    kind,
    requiredFields: ['travelerIds', 'origin', 'startDate', 'endDate', 'destination'],
    attachmentRequired: false,
    rejectResubmission: true,
    routes: [{ key: 'default', when: {}, nodes: [
      { ...blankNode('supervisor'), name: '上级领导审批', roleIds: supervisorRoleIds },
      { ...blankNode('project_leader'), name: '项目领导审批', roleIds: leadershipRoleIds, scope: 'institution', fixedUserIds: leadershipUserIds, projectDuty: 'concerned_leader' },
    ] }],
  }
  if (kind === '请假') return {
    kind,
    requiredFields: ['leaveType', 'startAt', 'endAt', 'hours'],
    attachmentRequired: false,
    rejectResubmission: true,
    routes: [{ key: 'default', when: {}, nodes: [{ ...blankNode('supervisor'), name: '上级领导审批', roleIds: supervisorRoleIds }] }],
  }
  if (kind !== '报销') return { kind, requiredFields: [], attachmentRequired: false, rejectResubmission: false, routes: [{ key: 'default', when: {}, nodes: [blankNode()] }] }
  return {
    kind,
    requiredFields: ['items', 'amount'],
    attachmentRequired: true,
    rejectResubmission: true,
    routes: [{ key: 'default', when: {}, nodes: [
      { ...blankNode('supervisor'), name: '直属上级审批', roleIds: byRole(role => role.fdeCategory === 'project_lead' || /部门负责人|投资总监|合伙人/.test(role.name)) },
      { ...blankNode('leadership'), name: '领导审批', roleIds: leadershipRoleIds, scope: 'institution', fixedUserIds: leadershipUserIds },
      { ...blankNode('finance'), name: '财务审批', roleIds: byRole(role => /财务/.test(role.name)), scope: 'institution' },
      { ...blankNode('cashier'), name: '出纳打款', roleIds: byRole(role => /出纳/.test(role.name)), scope: 'institution' },
    ] }],
  }
}
export function FdeOfficePolicyPanel() {
  ...
  return <div className="space-y-4"><Card className="fde-panel p-5"><div className="flex flex-wrap items-center justify-between gap-3"><h2 className="font-semibold">通用 OA 类型与审批规则</h2><div className="flex flex-wrap gap-2">{officeKinds.map(kind => <Button key={kind} variant="secondary" disabled={busy || blocked} onClick={() => open(undefined, kind)}>新建{kind}规则草稿</Button>)}<Button variant="secondary" disabled={busy} onClick={() => void work(refresh)}>刷新</Button></div></div></Card>
    ...
    {pending && <Card className="p-4"><p className="mb-3 text-sm">上一笔规则操作结果待核对。</p><Button disabled={busy || Boolean(storageError)} onClick={() => void recover()}>核对操作结果</Button></Card>}
