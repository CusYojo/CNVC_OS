...
export function FdeFridayMeetingPanel({ projectId }: { projectId: string }) {
  ...
  return <div className="fde-workspace space-y-4">
    <Card className="p-5"><div className="flex flex-wrap items-start justify-between gap-3"><div><h2 className="flex items-center gap-2 font-semibold"><CalendarDays className="h-5 w-5" />周五例会</h2><p className="mt-2 text-sm text-slate-500">议题与排期 → 人工确认结果、阻塞和决定 → 秘书生成下周计划草稿。不会自动完成任务或批准计划。</p></div><div className="flex gap-2"><Button variant="secondary" disabled={busy} onClick={() => { void reload().catch((cause) => setError((cause as Error).message)) }}><RefreshCw className="h-4 w-4" />刷新</Button>{data?.canManage && <Button disabled={busy} onClick={() => openEditor()}><Plus className="h-4 w-4" />新建例会</Button>}</div></div></Card>
    ...
    {data?.list.map((meeting) => <Card key={meeting.id} className="p-5"><div className="flex flex-wrap justify-between gap-3"><div><h3 className="font-semibold">{meeting.title}</h3><p className="mt-2 text-xs text-slate-500">{showTime(meeting.startedAt)} — {showTime(meeting.endsAt)}（上海） · 主持 {meeting.host} · v{meeting.version}</p><p className="mt-1 text-xs text-slate-500">参会：{meeting.participants.map((person) => person.sourceName).join('、')}</p></div><Badge tone={meeting.workflowStatus === 'completed' ? 'green' : 'slate'}>{status[meeting.workflowStatus]}</Badge></div>
    ...
    <Modal open={Boolean(detail)} title="例会版本历史" width="max-w-3xl" onClose={() => setDetail(null)}>{detail?.events.map((event) => <details key={event.id} className="border-b py-3 text-sm"><summary className="cursor-pointer">v{event.version} · {actions[event.action] ?? event.action} · {showTime(event.createdAt)} {event.reason}</summary><pre className="mt-3 whitespace-pre-wrap break-words text-xs text-slate-600">{JSON.stringify(event.snapshot.meeting.weeklyReview, null, 2)}</pre></details>)}</Modal>
  </div>
}
