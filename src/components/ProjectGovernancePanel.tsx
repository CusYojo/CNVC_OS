...
export function ProjectGovernancePanel({ projectId, onChanged }: { projectId: string; onChanged: () => Promise<void> }) {
  ...
  const name = (id: string) => data?.roster.find((person) => person.id === id)?.name ?? data?.eligiblePeople.find((person) => person.id === id)?.name ?? id.slice(0, 8)
