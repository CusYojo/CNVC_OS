...
type Props = { project: Project; files: ProjectFile[]; onChanged: () => unknown; approvalId?: string; onHandled?: () => Promise<void>; onBusyChange?: (value: boolean) => void; onDirtyChange?: (value: boolean) => void }
...
function RuntimePanel({ project, files, onChanged, uid, approvalId, onHandled, onBusyChange, onDirtyChange }: Props & { uid: string }) {
  const [searchParams] = useSearchParams(), requestedReview = approvalId ?? searchParams.get('typeReview'), returnCenter = searchParams.get('center')
  ...
  async function execute(body: Record<string, unknown>) {
    ...
    try { verifyTypeRuntimeReceipt(await apiPost(`${root}/commands`, command), marker); await finish('处理结果已保存'); if (command.action === 'decide') await onHandled?.() }
  ...
  return <Card className="fde-workspace fde-detail-type-flow space-y-4 p-5">
    {!approvalId && returnCenter !== null && <Link className="text-sm text-[#315f68]" to={approvalCenterReturnPath(returnCenter)}>返回统一审批中心</Link>}
    ...
    {data && <>
      ...
      {!approvalId && config?.actions.some(a => a.needLeader) && <section className="space-y-2 rounded-lg bg-slate-50 p-3 text-sm" aria-label="非投资计划领导时间">
      ...
      {!approvalId && config && <><div className="fde-detail-timeline" aria-label="项目流程" style={{ '--stage-count': config.stages.length } as CSSProperties}>{config.stages.map((item, index) => {
      ...
      {!approvalId && data.canPrepare && data.canWrite && (!instance || instance.status === 'draft') && <fieldset disabled={blocked} className="space-y-3 rounded-xl border p-4"><legend>计划编制</legend>
