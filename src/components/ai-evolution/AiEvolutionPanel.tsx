...
type WorkbenchStatus = 'pending' | 'running' | 'active' | 'history'
const workbenchLabels: Record<WorkbenchStatus, string> = { pending: '待处理', running: '进行中', active: '已生效', history: '历史' }
export function classifyEvolutionWorkbenchStatus(proposal: { status: string; spec: { kind: EvolutionKind } },
  state?: { runStatus?: string; candidateStatus?: string }): WorkbenchStatus {
  if (['draft', 'needs_input', 'ready'].includes(proposal.status)) return 'pending'
  if (['rejected', 'superseded'].includes(proposal.status)) return 'history'
  if (proposal.spec.kind === 'experience') return 'active'
  if (state?.candidateStatus === 'active') return 'active'
  if (['retired', 'failed', 'rolled_back'].includes(state?.candidateStatus ?? '')
    || ['failed', 'cancelled', 'interrupted'].includes(state?.runStatus ?? '')) return 'history'
  return 'running'
}
...
export function AiEvolutionPanel({ conversationId }: { conversationId: string }) {
  ...
  const visibleProposals = proposals.filter(proposal => (kind === 'all' || proposal.spec.kind === kind)
    && classifyEvolutionWorkbenchStatus(proposal, activity[proposal.id]) === workbenchStatus)
  return <section aria-label="自进化工作台" className="min-h-0 flex-1 overflow-y-auto p-3 text-sm">
    ...
    <div className="mb-3 grid grid-cols-4 gap-1" role="tablist" aria-label="进化状态">
      {(Object.keys(workbenchLabels) as WorkbenchStatus[]).map(value => <button key={value} role="tab" aria-selected={workbenchStatus === value}
        className={`rounded px-1 py-1.5 text-xs ${workbenchStatus === value ? 'bg-brand-600 text-white' : 'bg-slate-100 text-slate-600'}`}
    ...
    {(kind === 'all' || kind === 'experience') && ['active', 'history'].includes(workbenchStatus) && <details className="mb-4 rounded border border-slate-200 bg-white p-3">
      ...
      {experiences.filter(experience => workbenchStatus === 'active' ? experience.status === 'active' : experience.status !== 'active').map((experience) => <article key={experience.id} className="mt-3 border-t border-slate-100 pt-3">
    ...
    {!loading && !error && !visibleProposals.length && <p className="py-6 text-center text-slate-500">当前分类没有进化记录</p>}
    <div className="space-y-3">{visibleProposals.map((proposal) => <article key={proposal.id} className="rounded-lg border border-slate-200 bg-white p-3">
