  ...
  return <div className="fde-archive-page">
    <Card className="fde-panel fde-archive-library">
      ...
      {error ? <div className="fde-knowledge-state"><p role="alert">{error}</p><Button variant="secondary" onClick={() => setRefresh(value => value + 1)}>重新读取</Button></div> : !list ? <p role="status" className="fde-knowledge-state">正在读取有权项目档案…</p> : <>
        {!list.list.length ? <div className="fde-knowledge-state"><strong>当前范围暂无项目档案</strong></div> : grid ? <div className="fde-knowledge-grid">{list.list.map(file => <article className="fde-knowledge-card" key={file.id}><span className="fde-archive-type">{file.type}</span><h3>{file.name}</h3><p>{file.projectName} · {file.category}</p><div className="fde-knowledge-stats"><span>{file.uploader}</span><span>{when(file.uploadedAt)}</span></div><div className="fde-knowledge-actions"><Button size="sm" variant="secondary" onClick={() => open(file)}>详情</Button></div></article>)}</div> : <div className="fde-knowledge-table-scroll"><table className="fde-knowledge-table"><thead><tr>{['文件', '项目', '分类', '上传人', '权限', '操作'].map(title => <th key={title}>{title}</th>)}</tr></thead><tbody>{list.list.map(file => <tr key={file.id}>
    ...
    <Drawer open={Boolean(selected)} title="档案详情" width="w-[680px]" onClose={() => update({ file: null })}>
      {detailError ? <p role="alert" className="text-sm text-rose-600">{detailError}</p> : !detail ? <p>正在读取档案…</p> : <div className="space-y-5"><div><FileText className="mb-3 h-8 w-8 text-[#315f68]" /><h3 className="break-words text-lg font-semibold">{detail.file.name}</h3><p className="mt-2 text-sm text-slate-500">{detail.file.projectName} · {detail.file.category}</p><p className="mt-1 text-xs text-slate-400">{detail.file.uploader} · {when(detail.file.uploadedAt)}</p></div><div className="flex flex-wrap gap-2">{detail.file.hasOriginal && <a className="rounded-lg border border-slate-200 px-3 py-2 text-sm" target="_blank" rel="noopener noreferrer" href={`/api/projects/files/${detail.file.id}/preview`}>预览原件</a>}<Button disabled={!detail.file.canDownload || !detail.file.hasOriginal || busy} onClick={() => void download(detail.file)}><Download className="h-4 w-4" />下载原件</Button><Button variant="secondary" onClick={() => navigate(projectLink(detail.file))}>进入项目文件</Button>{detail.file.canAudit && <Button variant="secondary" onClick={() => { setAudit({ kind: 'access', fileId: detail.file.id, page: 1 }); update({ file: null }) }}>文件审计</Button>}</div><details className="rounded-xl border border-slate-200 bg-white p-4"><summary className="cursor-pointer text-sm font-semibold">访问范围</summary><p className="mt-3 text-sm text-slate-600">{detail.file.canDownload ? '当前可查看与下载' : '当前仅可查看'}</p></details><details className="rounded-xl border border-slate-200 bg-white p-4"><summary className="cursor-pointer text-sm font-semibold">文件信息</summary><div className="mt-3 text-sm leading-7 text-slate-600"><p>{size(detail.file.byteSize)} · 内容 V{detail.file.version}</p><p>解析状态：{detail.file.parseStatus}</p><p>{detail.file.hasOriginal ? '原件已登记' : '原件未登记'}</p></div></details><details className="rounded-xl border border-slate-200 bg-white p-4"><summary className="cursor-pointer text-sm font-semibold">历史版本</summary><div className="mt-3">{detail.versions.map(version => <div key={version.version} className="flex items-center justify-between gap-2 border-b border-slate-100 py-3 text-sm"><span>V{version.version} · {size(version.byteSize)}<small className="mt-1 block text-slate-400">{when(version.createdAt)}</small></span><Button variant="secondary" disabled={!detail.file.canDownload || busy} onClick={() => void download(detail.file, version.version)}>下载</Button></div>)}{!detail.total && <p className="text-sm text-slate-400">暂无历史版本</p>}<Pager {...detail} onChange={setVersionPage} /></div></details></div>}
    </Drawer>
