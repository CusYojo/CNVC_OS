...
function WeeklyReportForAccount({ initialWeek }: { initialWeek?: string }) {
  ...
  return <div className="fde-workspace space-y-4">
    ...
    {reports.map((report) => <Card key={report.id} className="fde-panel p-5"><div className="flex items-start justify-between gap-3"><h3 className="font-semibold">{report.own ? '我的周报' : '收到的周报'} R{report.revision}<span className="ml-2 text-xs font-normal text-slate-400">v{report.version}</span></h3><Badge tone={report.status === 'published' ? 'green' : 'slate'}>{labels[report.status]}</Badge></div>
      {report.restricted ? <p className="mt-4 text-sm text-amber-800">来源项目、日历、办公或独立工作权限已变化，正文与来源已隐藏。仍可撤回已发布内容或丢弃草稿。</p> : <>
        ...
        <p className="mt-3 text-xs text-slate-500">项目范围：{report.facts?.projects.map((item) => item.name).join('、') || '未选择项目，仅汇总所选个人来源'} · 来源采集：{report.facts && new Date(report.facts.generatedAt).toLocaleString('zh-CN', { timeZone: 'Asia/Shanghai' })}</p>
        <div className="mt-4 whitespace-pre-wrap rounded-lg bg-slate-50 p-4 text-sm leading-7 text-slate-700">{report.body}</div>
        <details className="mt-4"><summary className="cursor-pointer text-xs text-slate-500">统计口径与来源快照</summary><div className="mt-3 space-y-2 text-xs text-slate-600">{report.facts?.unavailable.map((item) => <p key={item}>{item}</p>)}{report.facts?.tasks.map((task) => <p key={task.id}>任务 {task.id} · v{task.version} · {task.title} · {task.status} · 期限 {task.dueDate ?? '未设置'} {task.dueTime ?? ''}</p>)}{report.facts?.approvals.map((item) => <p key={item.id}>审批记录 {item.id} · 申请 {item.requestId} · {item.action} · {item.occurredAt}</p>)}{report.facts?.meetings.map((item) => <p key={item.id}>会议 {item.id} · v{item.version} · {item.title} · {item.startedAt}</p>)}{report.facts?.calendar?.map(item => <p key={`${item.source}:${item.id}`}>日历来源 {item.source}:{item.id} · v{item.version} · {item.title} · {item.status} · {item.visibility === 'private' ? '受限来源' : '按来源权限'}</p>)}</div></details>
        {report.facts?.office?.map(item => <div key={item.id} className="mt-3 rounded-lg border border-slate-200 p-3 text-xs text-slate-600"><span>办公来源 · {item.kind} · 修订 {item.revision} · {item.status} · {item.actions.length} 次本人操作</span><Link className="ml-3 text-[#315f68] underline" to={`/workflow?office=${item.id}`}>打开原申请：{item.title}</Link></div>)}
        {report.facts?.milestones?.map(item => <div key={item.id} className="mt-3 rounded-lg border border-slate-200 p-3 text-xs text-slate-600"><span>{item.projectName} · {item.stage} · 批准日期 v{item.version} · {item.date}（非阶段通过）</span><Link className="ml-3 text-[#315f68] underline" to={milestoneSourceTarget(item)}>打开节点审批来源</Link></div>)}
      ...
      {report.events.length > 0 && <details className="mt-4"><summary className="cursor-pointer text-xs text-slate-500">周报版本与操作记录</summary>{report.events.map((event) => <p key={event.version} className="mt-2 text-xs text-slate-600">v{event.version} · {labels[event.action] ?? event.action} · {new Date(event.createdAt).toLocaleString('zh-CN', { timeZone: 'Asia/Shanghai' })} {event.reason}</p>)}</details>}
    </Card>)}
    <Modal open={Boolean(create)} title="生成个人周报草稿" onClose={() => { if (!busy) setCreate(null) }} footer={<><Button variant="secondary" disabled={busy} onClick={() => setCreate(null)}>取消</Button><Button loading={busy} disabled={!create || !(create.projectIds.length || create.sourceOptions.calendar || create.sourceOptions.independentWork || create.sourceOptions.office) || Boolean(create.sourceOptions.projectTimeline && !create.projectIds.length)} onClick={() => { if (create) void run(() => apiPost('/weekly-reports', { clientRequestId: create.requestId, projectIds: create.projectIds, sourceOptions: create.sourceOptions, weekStart: week }), '已生成事实草稿，尚未发布') }}>生成草稿</Button></>}>
      <p className="mb-4 text-sm text-slate-600">选择本次项目和来源。任务与会议按唯一来源统计，日历安排不代表完成或实际出席。私人日历默认不纳入；显式纳入私人内容或本人独立任务后，无来源权限的接收人不能读取。</p><div className="space-y-3">{projects.map((project) => <label key={project.id} className="flex items-center gap-2 text-sm"><input type="checkbox" checked={Boolean(create?.projectIds.includes(project.id))} onChange={(event) => { if (create) setCreate({ ...create, requestId: crypto.randomUUID(), projectIds: event.target.checked ? [...create.projectIds, project.id] : create.projectIds.filter((id) => id !== project.id) }) }} />{project.name}</label>)}</div>
