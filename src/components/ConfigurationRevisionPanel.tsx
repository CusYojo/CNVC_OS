...
export function ConfigurationRevisionPanel(props: {
  ...
  return <div className="fixed inset-0 z-[60] flex justify-end bg-slate-950/30" role="dialog" aria-modal="true" aria-label={`${target.resourceLabel} 配置历史`}>
    <div className="flex h-full w-full max-w-xl flex-col bg-white shadow-2xl">
      ...
      <div className="border-b border-blue-100 bg-blue-50 px-5 py-3 text-xs leading-5 text-blue-800"><ShieldCheck className="mr-1 inline h-3.5 w-3.5" />历史快照在服务端使用 AES-256-GCM 加密；此页面只读取版本、操作者、时间和完整性摘要，不读取凭据或快照正文。</div>
      ...
      <div className="flex-1 space-y-3 overflow-y-auto p-5">
        ...
        {revisions.map((revision) => <div key={revision.id} className="rounded-xl border border-slate-200 p-4">
          <div className="flex items-start justify-between gap-4"><div><p className="text-sm font-medium text-slate-700">{operationLabel[revision.operation]} · v{revision.sourceVersion}</p><p className="mt-1 flex items-center gap-1 text-xs text-slate-400"><Clock3 className="h-3 w-3" />{formatShanghaiDateTime(revision.createdAt)} · 操作者 {revision.createdBy ? revision.createdBy.slice(0, 8) : '已删除用户'}</p></div><button disabled={busy !== '' || (!revision.snapshotAvailable && revision.operation !== 'create')} className="inline-flex h-8 items-center gap-1 rounded-lg border border-brand-200 px-3 text-xs text-brand-600 disabled:cursor-not-allowed disabled:opacity-40" onClick={() => void rollback(revision)}><RotateCcw className="h-3.5 w-3.5" />{revision.operation === 'create' ? '撤销创建（停用）' : revision.snapshotAvailable ? '恢复此版本' : '快照不可用'}</button></div>
          <p className="mt-3 font-mono text-[11px] text-slate-400">SHA-256 {revision.snapshotSha256.slice(0, 16)}…</p>
        </div>)}
      </div>
    </div>
  </div>
