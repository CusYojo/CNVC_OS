...
export function FdeMaterialPanel({ projectId, onChanged }: { projectId: string; onChanged: () => void }) {
  ...
  const recovery = (pending || storageError) && <div ref={recoveryPanel} role="alert" className="mb-4 rounded-lg border border-amber-200 bg-amber-50 p-4 text-sm text-amber-900"><p>{storageError || recoveryError || '存在一笔尚未确认结果的送审操作，请先核对。'}</p>{pending && <Button className="mt-3" variant="secondary" disabled={busy} onClick={resolvePending}>核对提交结果</Button>}</div>
  ...
  return <div className="fde-workspace mt-5">
    ...
    <Card className="fde-panel overflow-hidden">
      ...
      {error ? <p role="alert" className="p-5 text-sm text-red-600">{error}</p> : !list ? <p className="p-5 text-sm text-slate-500">正在读取送审记录…</p> : <>
        <div className="overflow-x-auto"><table className="fde-plan-table"><thead><tr><th>材料 / 送审标题</th><th>发送人</th><th>逐人进度</th><th>状态</th><th>操作</th></tr></thead><tbody>{list.list.map(item => <tr key={item.id}><td><p className="font-medium">{item.title}</p><p className="mt-1 text-xs text-slate-400">{item.file ? `${item.file.name} · 第 ${item.revision} 轮` : '文件权限已失效，正文隐藏'}</p></td><td>{item.senderName}</td><td>{item.restricted ? '—' : `${item.recipients.filter(person => person.readAt).length}/${item.recipients.length} 已读 · ${item.recipients.filter(person => person.decision).length}/${item.recipients.length} 已反馈`}</td><td><Badge tone={item.status === 'approved' ? 'green' : item.status.includes('return') ? 'red' : 'slate'}>{item.statusLabel}</Badge></td><td><Button variant="secondary" onClick={() => open(item.id)}>查看送审</Button></td></tr>)}</tbody></table></div>
    ...
    <Drawer open={Boolean(selected)} title="送审材料详情" onClose={close} width="w-full sm:w-[660px]" footer={<div className="flex flex-wrap gap-2">{row?.capabilities.withdraw && <Button variant="secondary" disabled={blocked} onClick={() => { setAction('withdraw'); setFeedback(''); changeRequest() }}>撤回送审</Button>}{row?.capabilities.resubmit && <Button variant="secondary" disabled={blocked} onClick={() => beginCreate(row)}>补充后新轮次送审</Button>}{row?.capabilities.decide && <><Button variant="secondary" disabled={blocked} onClick={() => { setAction('return'); setFeedback(''); changeRequest() }}>退回补充</Button><Button disabled={blocked} onClick={() => { setAction('approve'); setFeedback(''); changeRequest() }}>批复通过</Button></>}<Button variant="secondary" disabled={busy} onClick={close}>完成</Button></div>}>
      ...
      {detailError ? <p role="alert" className="text-sm text-red-600">{detailError}</p> : !row ? <p className="text-sm text-slate-500">正在读取送审与当前权限…</p> : <div className="space-y-5">
        ...
        {row.file && <section className="rounded-xl border border-[#dfe6e4] p-4"><h4 className="break-all text-sm font-medium">{row.file.name}</h4>{row.file.currentVersion !== row.file.version && <p className="mt-2 text-sm text-amber-800">文件已有更新，本轮批复仍使用送审版本。</p>}<div className="mt-3 flex flex-wrap gap-3 text-sm text-[#315f68]"><a href={`/api${base}/${row.id}/preview`} target="_blank" rel="noopener noreferrer">预览送审文件</a>{row.capabilities.download && <a href={`/api/projects/files/${row.file.id}/versions/${row.file.version}/download`}>下载送审文件</a>}</div></section>}
        ...
        {detail && !row.restricted && <details className="rounded-xl border border-[#dfe6e4] p-4"><summary className="cursor-pointer font-semibold">操作记录</summary><ol className="mt-3 space-y-3">{detail.events.map(event => <li key={event.id} className="border-l-2 border-[#dfe6e4] pl-3 text-xs text-slate-500"><p>{event.actorName} · {actions[event.action] ?? event.action}</p><p className="mt-1 whitespace-pre-wrap">{event.reason}</p><p className="mt-1">{when(event.createdAt)}</p></li>)}</ol><div className="mt-3 flex items-center justify-between gap-2 text-xs text-slate-400"><span>共 {detail.historyTotal} 条</span><div className="flex gap-2"><Button variant="secondary" disabled={detail.page <= 1 || busy} onClick={() => void load(row.id, false, detail.page - 1).catch(() => {})}>上一页</Button><Button variant="secondary" disabled={detail.page * detail.pageSize >= detail.historyTotal || busy} onClick={() => void load(row.id, false, detail.page + 1).catch(() => {})}>下一页</Button></div></div></details>}
      </div>}
    </Drawer>
    <Modal open={createOpen} title={previousId ? '补充后新轮次送审' : '送审材料'} onClose={() => { if (!busy) setCreateOpen(false) }} width="max-w-2xl" footer={<><Button variant="secondary" disabled={blocked} onClick={() => setCreateOpen(false)}>取消</Button><Button loading={busy} disabled={blocked || createStale || !context?.canSubmit || !chosenFile?.hasOriginal || !title.trim() || !recipientIds.length} onClick={submit}>确认送审</Button></>}>
      ...
      <div className="space-y-4">{createError && <p role="alert" className="text-sm text-red-600">{createError}</p>}{createStale && <div className="rounded-lg border border-amber-200 bg-amber-50 p-3"><p className="mb-2 text-xs text-amber-900">请刷新文件与接收人后重新选择。标题和说明会保留，不会自动重新送审。</p><Button variant="secondary" disabled={blocked} onClick={refreshCreateContext}>刷新文件与接收人</Button></div>}
        ...
        {chosenFile && <p className="text-xs text-[#315f68]">已选：{chosenFile.name}</p>}
