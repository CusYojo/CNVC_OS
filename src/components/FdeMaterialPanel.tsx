...
export function FdeMaterialPanel({ projectId, onChanged }: { projectId: string; onChanged: () => void }) {
  ...
  return <div className="fde-workspace mt-5">
    ...
    <Card className="fde-panel overflow-hidden">
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-[#dfe6e4] p-5"><div><h2 className="font-semibold">材料送审</h2><p className="mt-1 text-xs text-slate-500">逐人送达与反馈，保存送审文件版本；不代替正式审批。</p></div><div className="flex gap-2"><Button variant="secondary" onClick={() => setRefresh(n => n + 1)}><RefreshCw className="h-4 w-4" />刷新送审</Button>{list?.canSubmit && <Button disabled={blocked} onClick={() => beginCreate()}><Send className="h-4 w-4" />送审材料</Button>}</div></div>
    ...
    <Modal open={createOpen} title={previousId ? '补充后新轮次送审' : '送审材料'} onClose={() => { if (!busy) setCreateOpen(false) }} width="max-w-2xl" footer={<><Button variant="secondary" disabled={blocked} onClick={() => setCreateOpen(false)}>取消</Button><Button loading={busy} disabled={blocked || createStale || !context?.canSubmit || !chosenFile?.hasOriginal || !title.trim() || !recipientIds.length} onClick={submit}>确认送审</Button></>}>
      {recovery}
      <div className="space-y-4">{createError && <p role="alert" className="text-sm text-red-600">{createError}</p>}{createStale && <div className="rounded-lg border border-amber-200 bg-amber-50 p-3"><p className="mb-2 text-xs text-amber-900">请刷新文件与接收人后重新选择。标题和说明会保留，不会自动重新送审。</p><Button variant="secondary" disabled={blocked} onClick={refreshCreateContext}>刷新文件与接收人</Button></div>}<p className="text-xs leading-5 text-slate-500">只为指定接收人补足查看权，不自动授予下载权。保存本轮原始文件版本，逐人反馈不代替项目阶段审批。</p>
