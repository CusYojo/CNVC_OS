...
export function PageHeader({ title, description, actions }: { title: string; description?: string; actions?: ReactNode }) {
  return (
    <div className="fde-ui-page-header mb-5 flex min-h-14 items-center justify-between">
      ...
      {actions && <div className="fde-ui-page-header-actions flex items-center gap-2">{actions}</div>}
    </div>
  )
}
...
export function Modal({ open, title, children, onClose, width = 'max-w-xl', footer }: { open: boolean; title: string; children: ReactNode; onClose: () => void; width?: string; footer?: ReactNode }) {
  if (!open) return null
  return (
    <div className="fde-ui-modal-backdrop fixed inset-0 z-50 flex items-center justify-center bg-slate-900/30 p-6 backdrop-blur-[2px]" onMouseDown={(event) => event.target === event.currentTarget && onClose()}>
      <div role="dialog" aria-modal="true" aria-label={title} className={`fde-ui-modal max-h-[88vh] w-full max-w-[calc(100vw-2rem)] ${width} overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-2xl`}>
        ...
        <div className="fde-ui-modal-body max-h-[calc(88vh-132px)] overflow-y-auto px-6 py-5 scrollbar-thin">{children}</div>
        {footer && <div className="fde-ui-modal-footer flex flex-wrap items-center justify-end gap-2 border-t border-slate-100 bg-slate-50/60 px-6 py-4">{footer}</div>}
      </div>
    </div>
  )
}
...
export function Drawer({ open, title, children, onClose, width = 'w-[560px]', footer }: { open: boolean; title: string; children: ReactNode; onClose: () => void; width?: string; footer?: ReactNode }) {
  if (!open) return null
  return (
    <div className="fde-ui-drawer-backdrop fixed inset-0 z-50 bg-slate-900/25 backdrop-blur-[1px]" onMouseDown={(event) => event.target === event.currentTarget && onClose()}>
      <aside role="dialog" aria-modal="true" aria-label={title} className={`fde-ui-drawer absolute right-0 top-0 flex h-full max-w-full ${width} flex-col bg-white shadow-2xl`}>
        ...
        <div className="fde-ui-drawer-body flex-1 overflow-y-auto px-6 py-5 scrollbar-thin">{children}</div>
        {footer && <div className="fde-ui-drawer-footer flex flex-wrap items-center justify-end gap-2 border-t border-slate-100 bg-slate-50 px-6 py-4">{footer}</div>}
      </aside>
    </div>
  )
}
...
export function DataTable({ headers, children }: { headers: string[]; children: ReactNode }) {
  return (
    <div className="fde-ui-table-scroll overflow-x-auto">
