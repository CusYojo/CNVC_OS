...
type PolicyVersion = { id: string; revision: number; status: string; version: number; reason: string; configuration: FdeWorkflowPolicyConfig; sha256: string; boundProjects: number; publishedAt: string | null }
...
export function FdePolicyPanel() {
  ...
  return <div className="space-y-4">
    <div className="flex flex-wrap items-center justify-between gap-3"><p className="max-w-3xl text-xs leading-6 text-slate-500">项目绑定发布时的固定规则版本。新版本只影响后续新项目；原 AI 模板和线索池模板在各自原入口使用，不在这里覆盖。</p><Button variant="secondary" loading={loading} onClick={() => void refresh()}><RefreshCw className="h-4 w-4" />刷新规则</Button></div>
    {error && <Card className="p-5 text-sm text-red-600">规则读取失败：{error}</Card>}
    {!loading && !error && !policies.length && <Card className="p-5 text-sm text-slate-500">暂无规则模板，请检查已批准的数据库迁移。</Card>}
    {policies.map((policy) => <Card className="fde-panel p-5" key={policy.id}>
      ...
      <div className="mt-5 space-y-3">{policy.versions.map((version) => <div key={version.id} className="flex flex-wrap items-center gap-3 rounded-lg border border-slate-200 p-4"><div className="min-w-0 flex-1"><p className="text-sm font-medium">V{version.revision} <Badge tone={version.status === 'draft' ? 'amber' : 'green'}>{version.status === 'draft' ? '草稿' : version.id === policy.activeVersionId ? '当前发布' : '历史发布'}</Badge></p><p className="mt-2 text-xs text-slate-500">{version.reason}</p><p className="mt-1 text-xs text-slate-400">绑定项目 {version.boundProjects} 个 · SHA-256 {version.sha256.slice(0, 12)}{version.publishedAt ? ` · ${formatShanghaiDateTime(version.publishedAt)}` : ''}</p></div><Button variant="secondary" onClick={() => open(version)}>{version.status === 'draft' ? '编辑草稿' : '查看规则'}</Button><Button variant="secondary" onClick={() => { setReason(''); setAction({ kind: 'clone', policy, version }) }}><FilePlus2 className="h-4 w-4" />基于此版新建</Button>{version.status === 'draft' && <Button onClick={() => { setReason(''); setAction({ kind: 'publish', policy, version }) }}>预览并发布</Button>}</div>)}</div>
    </Card>)}
