...
export function FdeOfficeExecutionPanel({ id, version, canWrite, attachments, run, write, reload }: Props) {
  ...
  return <section className="mt-6 space-y-3 border-t pt-5">
    <h3 className="font-semibold">执行回执</h3>
    ...
    {view && <>
      ...
      {!view.records.length && !view.hiddenRecords && <p className="text-sm text-slate-500">暂无执行回执</p>}
      {view.records.map(record => <Card key={record.id} className="space-y-2 p-3 text-xs">
        <p className="font-semibold">{officeExecutionLabels[record.action]} · {record.outcome === 'succeeded' ? '办理完成' : '办理失败'}</p>
        <p>办理时间 {formatShanghaiDateTime(record.occurredAt)} · 登记人 {record.actorName} · 登记时间 {formatShanghaiDateTime(record.recordedAt)}</p>
        {record.supersedesId && <p>已更正上一条记录</p>}
        ...
        {record.files.map(file => <p key={file.fileId}><a className="text-teal-800" href={`/api${base}/attachments/${file.fileId}/preview`} target="_blank" rel="noreferrer">{file.name}</a></p>)}
      </Card>)}
      ...
      {view.canRecord && <fieldset disabled={disabled} className="space-y-3 rounded-lg border p-4">
        ...
        <p className="text-xs text-slate-500">附件权限可在上方管理。</p>
        ...
        <label className="flex gap-2 text-xs"><input type="checkbox" checked={confirmed} onChange={e => setConfirmed(e.target.checked)} />已核对办理结果和相关原件</label>
