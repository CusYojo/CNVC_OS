...
type ReplanProps = { projectId: string; onChanged?: () => Promise<void>; approvalId?: string; onHandled?: () => Promise<void>; onBusyChange?: (value: boolean) => void; onDirtyChange?: (value: boolean) => void }
export function FdeProjectReplanPanel(props: ReplanProps) {
  const { projectId } = props
  const userId = useAuthStore(s => s.user?.id ?? '')
  return userId ? <ReplanWorkspace key={`${userId}:${projectId}`} {...props} userId={userId} /> : null
}
function ReplanWorkspace({ projectId, userId, onChanged, approvalId, onHandled, onBusyChange, onDirtyChange }: ReplanProps & { userId: string }) {
  const endpoint = `/projects/${projectId}/project-replans`, key = replanRecoveryKey(userId, projectId)
  const [params] = useSearchParams(), selected = approvalId ?? params.get('replan')
  ...
  useEffect(() => { if (!approvalId && selected && data?.requests.some(r => r.id === selected)) document.getElementById(`project-replan-${selected}`)?.scrollIntoView({ block: 'center' }) }, [selected, data])
  ...
  async function act() {
    ...
    await run(async () => {
      ...
      clearReplanPending(localStorage, key, marker); await refreshed('处理结果已保存'); await onHandled?.()
    })
  }
  return <Card className="min-w-0 border-[#dfe6e4] p-5" aria-label="项目整体重排">
    ...
    {!approvalId && params.has('center') && <Link className="mt-2 block text-sm text-[#315f68]" to={approvalCenterReturnPath(params.get('center'))}>返回审批列表</Link>}
    ...
    {data && <><p className="mt-3 text-sm">当前目标日：{data.targetDate ?? '未配置'}</p>{data.policyIssue && <p className="mt-2 text-sm text-amber-800">{data.policyIssue}</p>}
      {!approvalId && <><form className="mt-3 space-y-3" onSubmit={e => { e.preventDefault(); void run(async () => { const token = generation.current; const value = await apiPost<ReplanPreview>(`${endpoint}/preview`, { targetDate }); if (current() && token === generation.current) setPreview(value) }) }}>
      ...
      {approvalId && !data.requests.some(row => row.id === approvalId) && <p role="alert" className="mt-4 text-sm">该审批已不可访问，请返回待办列表核对</p>}
      <div className="mt-5 space-y-3">{data.requests.filter(row => !approvalId || row.id === approvalId).map(row => <article key={row.id} id={`project-replan-${row.id}`} tabIndex={-1} className={`rounded-lg border p-3 ${selected === row.id ? 'border-[#315f68] ring-2 ring-[#a8c5c3]' : 'border-slate-200'}`}>
    ...
    <Modal open={Boolean(action)} title="办理整体重排" onClose={() => { if (!busy) setAction(null) }}><p className="mb-3 text-sm">请核对新时间与相关日程。</p><label className="block text-sm">办理意见<textarea className="input mt-1 w-full" minLength={6} maxLength={1000} value={reason} disabled={blocked} onChange={e => setReason(e.target.value)} /></label><Button disabled={blocked || reason.trim().length < 6} onClick={() => void act()}>确认办理</Button></Modal>
  </Card>
}
