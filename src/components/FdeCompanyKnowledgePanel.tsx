...
function KnowledgeAccountPanel({ userId }: { userId: string }) {
  ...
  const finish = (marker: KnowledgePending, version?: number) => {
    ...
    const message = version ? `原操作已保存至版本 v${version}。详情仍按当前权限读取。` : '原请求确认未提交，已封闭迟到写入。请重新读取当前内容并确认后再操作。'
  ...
  return <div className="fde-workspace">
      ...
      {list && <>
        <div className="fde-knowledge-grid">{list.list.map(item => <article className="fde-knowledge-card" key={item.id}>
          <button className="fde-knowledge-card-main" disabled={blocked} onClick={() => navigateEntry(item.id)} aria-label={`查阅：${item.title}`}>
            <div className="fde-knowledge-meta"><Badge tone="blue">{item.kind}</Badge><span>{item.audience === 'company' ? '全公司可见' : '指定人员可见'}</span></div>
            <h3>{item.title}</h3><p>{item.summary}</p>
            <div className="fde-knowledge-stats"><span>{item.authorName}</span><span>{new Date(item.updatedAt).toLocaleDateString('zh-CN', { timeZone: 'Asia/Shanghai' })}</span></div>
          </button>
          <div className="fde-knowledge-actions">
            ...
            {item.status !== 'published' && <span className="fde-knowledge-lifecycle">{labels[item.status]}</span>}
          </div>
        </article>)}</div>
    ...
    {pending && !busy && <div role="alert" className="mt-4 rounded-xl border border-amber-300 bg-amber-50 p-4 text-sm">提交结果尚未确认。刷新后仍可核对；不会自动重放原请求。<Button className="ml-3" variant="secondary" onClick={() => void recover()}>核对原请求结果</Button></div>}
    {selected && !editor && !interaction && <div className="fixed inset-0 z-40 bg-slate-900/25" onClick={close}><aside role="dialog" aria-modal="true" aria-label="知识详情" className="absolute bottom-0 right-0 top-0 flex w-full max-w-2xl flex-col bg-[#f4f6f5] shadow-xl" onClick={e => e.stopPropagation()}>
      ...
      <div className="flex-1 space-y-5 overflow-y-auto p-5">{detailError && <p role="alert" className="text-sm text-red-600">{detailError}</p>}{!detail && !detailError && <p>正在读取…</p>}{detail && entry && <>
        <Card className="fde-panel p-5"><h3 className="text-sm font-semibold">内容摘要</h3><p className="mt-3 whitespace-pre-wrap break-words text-sm leading-7">{entry.summary}</p><p className="mt-3 text-xs text-slate-500">{entry.authorName} · {new Date(entry.updatedAt).toLocaleDateString('zh-CN', { timeZone: 'Asia/Shanghai' })}</p>{entry.link && <a className="mt-4 inline-flex items-center gap-1 text-sm text-[#315f68]" href={entry.link} target="_blank" rel="noopener noreferrer"><ExternalLink className="h-4 w-4" />打开原文</a>}</Card>
        {entry.body && <Card className="fde-panel p-5"><h3 className="text-sm font-semibold">文章正文</h3><div className="mt-3 whitespace-pre-wrap break-words text-sm leading-7">{entry.body}</div></Card>}
        <details className="rounded-xl border border-slate-200 bg-white p-4"><summary className="cursor-pointer text-sm font-semibold text-slate-700">访问范围</summary><div className="mt-3 space-y-2 text-sm text-slate-600"><p>{entry.audience === 'company' ? '全公司可见' : '指定人员可见'}</p><p>{entry.capabilities.download ? '当前账号可下载' : '当前账号可查看'}</p></div></details>
        <details className="rounded-xl border border-slate-200 bg-white p-4"><summary className="cursor-pointer text-sm font-semibold text-slate-700">文件信息</summary><div className="mt-3 text-sm text-slate-600">{entry.file ? <><p>{entry.file.name} · V{entry.file.version}</p><div className="mt-3 flex flex-wrap gap-3"><a className="text-[#315f68]" href={`/api${endpoint}/${entry.id}/preview`} target="_blank" rel="noopener noreferrer">查看附件</a>{entry.capabilities.download && <a className="text-[#315f68]" href={`/api${endpoint}/${entry.id}/download`}>下载附件</a>}<a className="text-[#315f68]" href={`/api${endpoint}/${entry.id}/summary`}>下载摘要</a></div></> : <p>未关联文件</p>}</div></details>
        ...
        <details className="rounded-xl border border-slate-200 bg-white p-4"><summary className="cursor-pointer text-sm font-semibold text-slate-700">版本与操作记录</summary><div className="mt-3 space-y-2">{detail.history.map(item => <p className="text-xs leading-6 text-slate-500" key={item.id}>v{item.version} · {labels[item.action] ?? item.action} · {item.actorName} · {item.reason}</p>)}<div className="flex gap-2"><Button variant="secondary" disabled={busy || blocked || detail.historyPage <= 1} onClick={() => void loadDetail(entry.id, detail.page, detail.historyPage - 1)}>上页</Button><Button variant="secondary" disabled={busy || blocked || detail.historyPage * detail.pageSize >= detail.historyTotal} onClick={() => void loadDetail(entry.id, detail.page, detail.historyPage + 1)}>下页</Button></div></div></details>
