...
function ResponsibilityAccountPanel({ uid, management }: { uid: string; management: boolean }) {
  ...
  const recovery = <>{storageError && <div role="alert" className="mb-3 rounded-lg bg-red-50 p-3 text-sm">{storageError}<Button variant="secondary" disabled={busy} onClick={readRecovery}>重新读取恢复标识</Button></div>}{notice && <p role="status" className="mb-3 break-words rounded-lg bg-slate-50 p-3 text-sm">{notice}</p>}{pending && <div className="mb-3 rounded-lg border border-amber-300 bg-amber-50 p-3 text-sm"><p>有一笔{actionNames[pending.action] ?? '通知已读'}操作结果待确认，不能重复提交。</p><Button className="mt-2" variant="secondary" disabled={busy} onClick={() => void recover()}>{busy ? '正在核对…' : '核对原操作结果'}</Button></div>}</>
  ...
  return <div className="fde-workspace fde-responsibility-workspace space-y-4">
    ...
    <div className="fde-responsibility-layout">
      ...
      {management && <Card className="fde-panel fde-score-rules"><div className="fde-card-head"><h3 className="font-semibold">责任记录规则</h3></div>{allowedManagement && <FdeResponsibilityCurrentPolicy refresh={refresh} compact />}</Card>}
    </div>
    ...
    <Modal open={Boolean(selected)} title={action === 'appeal' ? '责任记录申诉' : action === 'review' ? '复核责任记录申诉' : action === 'confirm' ? '确认责任候选' : action === 'reroute' ? '重新核对处理人' : '责任记录详情'} onClose={close} width="max-w-5xl" footer={<Button variant="secondary" disabled={busy} onClick={close}>返回台账</Button>}>
      ...
      {detail && record && allowedManagement && <div className="space-y-5">
        <section className="rounded-lg border p-3 text-xs leading-6 text-slate-500"><h3 className="font-medium text-slate-700">原触发来源</h3><p>{eventName(record.eventCode)} · {when(record.occurredAt)}</p><p>关联任务已留档{record.sourceFeedbackId ? ' · 含任务反馈' : ''}{record.sourceAcceptanceId ? ' · 含成果验收' : ''}{record.sourceRiskId ? ' · 含风险记录' : ''}</p></section>
        ...
        <details className="rounded-lg border p-4 text-sm"><summary className="cursor-pointer font-medium">记录绑定规则与版本</summary><p className="my-3 text-xs">该记录已绑定创建时的历史规则版本。</p><div className="grid gap-2 sm:grid-cols-2">{detail.policy.rules.map(rule => <div key={rule.code} className="rounded-lg bg-slate-50 p-2">{eventName(rule.code)} · {rule.mode === 'automatic' ? '自动' : '人工确认'} · {rule.enabled ? points(rule.points ?? 0) : '未启用'}</div>)}</div><p className="mt-3">申诉中汇总：{detail.policy.appealAggregation === 'exclude_pending' ? '暂不计入' : '保留原有效值'}；历史修订保留，不自动重算。</p></details>
