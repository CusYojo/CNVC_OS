...
export function FdeFilePanel({ projectId, refreshKey, initialFileId, onUpload, onReplace, onChanged }: { projectId: string; refreshKey: string; initialFileId?: string | null; onUpload: () => void; onReplace: (file: ProjectFile) => void; onChanged: () => void | Promise<void> }) {
  ...
  return <div className="fde-workspace">
    <Card className="fde-panel overflow-hidden">
      <div className="fde-detail-file-head flex flex-wrap items-center justify-between gap-3 border-b border-[#dfe6e4] p-5"><div><h2 className="font-semibold">项目文件</h2><p className="mt-1 text-xs text-slate-500">查看与下载分别授权，材料版本、权限变更与回收操作全程留痕。</p></div><div className="flex gap-2"><Button variant="secondary" onClick={() => setRefresh(value => value + 1)}><RefreshCw className="h-4 w-4" />刷新文件</Button>{list?.canUpload && view === 'active' && <Button onClick={onUpload}><Upload className="h-4 w-4" />上传资料</Button>}</div></div>
      ...
      {error ? <p role="alert" className="p-5 text-sm text-red-600">文件加载失败：{error}</p> : !list ? <p className="p-5 text-sm text-slate-500">正在读取有权访问的文件…</p> : <>
        <div className="fde-detail-file-grid">{list.list.map(file => <article className="fde-detail-file-card" key={file.id}><button onClick={() => open(file.id)}><span className="fde-detail-file-icon">{file.name.split('.').pop()?.slice(0, 4).toUpperCase() || 'FILE'}</span><span><strong>{file.name}</strong><small>{file.category} · V{file.version}</small><small>{bytes(file.byteSize)} · {file.uploader}</small></span></button><footer><StatusBadge status={file.parseStatus} /><button onClick={() => open(file.id)}>{view === 'deleted' ? '查看回收详情' : '查看详情'} →</button></footer></article>)}</div>
    ...
    <Drawer open={Boolean(selected)} title="文件详情" onClose={close} width="w-full sm:w-[620px]" footer={<div className="flex flex-wrap gap-2">{detail?.capabilities.trash && <Button variant="secondary" disabled={busy || detail.blockers.length > 0} onClick={() => chooseAction('trash')}><Trash2 className="h-4 w-4" />移入回收站</Button>}{detail?.capabilities.restore && <Button variant="secondary" disabled={busy} onClick={() => chooseAction('restore')}><ArchiveRestore className="h-4 w-4" />恢复文件</Button>}{detail?.capabilities.manage && detail.file.lifecycle === 'active' && <Button variant="secondary" disabled={busy} onClick={() => chooseAction('permissions')}><ShieldCheck className="h-4 w-4" />设置权限</Button>}<Button disabled={busy} onClick={close}>完成</Button></div>}>
      {detailError ? <p role="alert" className="text-sm text-red-600">{detailError}</p> : !detail ? <p className="text-sm text-slate-500">正在读取文件详情…</p> : <div className="space-y-5" role="region" aria-label="文件详情内容">
        <section className="rounded-xl border border-[#dfe6e4] p-4"><h3 className="break-all font-semibold">{detail.file.name}</h3><p className="mt-1 text-xs text-slate-400">{detail.file.id}</p><dl className="mt-4 grid grid-cols-2 gap-4 text-sm"><div><dt className="text-xs text-slate-400">分类 / 版本</dt><dd>{detail.file.category} · V{detail.file.version}</dd></div><div><dt className="text-xs text-slate-400">大小 / 上传人</dt><dd>{bytes(detail.file.byteSize)} · {detail.file.uploader}</dd></div><div><dt className="text-xs text-slate-400">文件状态</dt><dd>{detail.file.lifecycle === 'active' ? '有效文件' : '已移入回收站'}</dd></div><div><dt className="text-xs text-slate-400">下载权限</dt><dd>{detail.capabilities.download ? '允许下载' : '当前未授予下载权'}</dd></div></dl><p className="mt-4 break-all text-xs text-slate-400">SHA-256：{detail.file.sha256 ?? '尚未校验'}</p></section>
        ...
        {detail.file.lifecycle === 'deleted' && <section className="rounded-lg bg-slate-50 p-3 text-sm"><p>回收原因：{detail.file.deleteReason}</p><p className="mt-2">保留截止：{when(detail.file.retentionUntil)}</p><p className="mt-2 text-xs text-slate-500">恢复前验证当前项目、权限及全部历史原件；到期不代表自动清除原字节。</p></section>}
        {detail.capabilities.view && <section><h3 className="font-semibold">内容版本</h3>{versionError ? <p role="alert" className="mt-2 text-sm text-red-600">{versionError}</p> : <ul className="mt-3 space-y-2">{versions.map(item => <li key={item.version} className="flex items-center justify-between gap-3 rounded-lg border border-[#dfe6e4] p-3 text-xs"><span>V{item.version} · {bytes(item.byteSize)}<br />{when(item.createdAt)}</span>{detail.capabilities.download && <button className="text-[#315f68]" disabled={busy} onClick={() => { void download(item.version) }}>下载 V{item.version}</button>}</li>)}</ul>}</section>}
        <section><h3 className="font-semibold">权限与操作记录</h3><ol className="mt-3 space-y-3">{detail.events.map(event => <li key={event.id} className="border-l-2 border-[#dfe6e4] pl-3 text-sm"><p>{event.actorName} · {eventNames[event.action] ?? event.action} · V{event.version}</p><p className="mt-1 whitespace-pre-wrap text-slate-500">{event.reason}</p><p className="mt-1 text-xs text-slate-400">{when(event.createdAt)}</p></li>)}</ol>{!detail.events.length && <p className="mt-2 text-sm text-slate-400">暂无权限或生命周期变更</p>}<div className="mt-3 flex items-center justify-between gap-2 text-xs text-slate-500"><span>共 {detail.historyTotal} 条操作记录</span><div className="flex gap-2"><button disabled={detail.page <= 1 || busy} onClick={() => { void load(detail.file.id, detail.page - 1).catch(() => {}) }}>上一页</button><button disabled={detail.page * detail.pageSize >= detail.historyTotal || busy} onClick={() => { void load(detail.file.id, detail.page + 1).catch(() => {}) }}>下一页</button></div></div></section>
      </div>}
    </Drawer>
    <Modal open={Boolean(action && detail)} title={action === 'permissions' ? '设置文件权限' : action === 'trash' ? '移入回收站' : '恢复文件'} onClose={() => { if (!busy) setAction(null) }} width="max-w-2xl" footer={<><Button variant="secondary" disabled={busy} onClick={() => setAction(null)}>取消</Button><Button loading={busy} disabled={reason.trim().length < 5} onClick={() => { void submit() }}>确认{action === 'permissions' ? '保存权限' : action === 'trash' ? '回收' : '恢复'}</Button></>}>
      {detail && <div className="space-y-4"><p className="break-all text-sm font-medium">{detail.file.name}</p>{action === 'permissions' ? <><p className="text-xs leading-5 text-slate-500">查看和下载分别设置。勾选下载同时勾选查看；当前操作人和有效上传人的查看权保留。最终权限还受当前项目职责和账号状态限制。</p><div className="overflow-x-auto"><table className="w-full text-sm"><thead><tr className="border-b text-left"><th className="p-2">项目人员</th><th className="p-2">查看</th><th className="p-2">下载</th></tr></thead><tbody>{detail.people.map(person => { const grant = grants.find(item => item.userId === person.id); return <tr key={person.id} className="border-b border-slate-100"><td className="p-2">{person.name}<span className="ml-2 text-xs text-slate-400">{person.owner ? '负责人' : person.secretary ? '秘书' : person.leader ? '领导' : '项目人员'}</span></td><td className="p-2"><input aria-label={`${person.name}查看权`} type="checkbox" disabled={busy} checked={Boolean(grant?.canView)} onChange={event => changeGrant(person.id, 'canView', event.target.checked)} /></td><td className="p-2"><input aria-label={`${person.name}下载权`} type="checkbox" disabled={busy} checked={Boolean(grant?.canDownload)} onChange={event => changeGrant(person.id, 'canDownload', event.target.checked)} /></td></tr> })}</tbody></table></div></> : <p className="text-sm text-slate-500">{action === 'trash' ? '回收后退出正常列表、预览、下载和检索。受保护引用会阻止回收，原始字节和历史版本不会删除。' : '在保留期内恢复原文件 ID；恢复前校验所有原件及当前权限，不重新启动已结束的审批或任务。'}</p>}<label className="block"><span className="label">操作原因（至少 5 字）</span><textarea className="textarea min-h-24" maxLength={600} disabled={busy} value={reason} onChange={event => { setReason(event.target.value); setRequestId(crypto.randomUUID()) }} /></label></div>}
    </Modal>
  </div>
}
