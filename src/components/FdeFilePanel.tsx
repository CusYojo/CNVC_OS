...
type FileItem = ProjectFile & { byteSize: number; sha256: string | null; accessVersion: number; accessMode: string; lifecycle: string; deletedAt: string | null; deleteReason: string | null; retentionUntil: string | null; capabilities?: { trash: boolean; restore: boolean } }
...
export function FdeFilePanel({ projectId, refreshKey, initialFileId, onUpload, onReplace, onChanged }: { projectId: string; refreshKey: string; initialFileId?: string | null; onUpload: () => void; onReplace: (file: ProjectFile) => void; onChanged: () => void | Promise<void> }) {
  ...
  const chooseListAction = async (id: string, value: 'trash' | 'restore') => {
    if (busy) return
    setSelected(id); setDetail(null); setDetailError(''); setAction(null)
    const loaded = await load(id).catch(() => null)
    if (!loaded) return
    if (!loaded.capabilities[value]) { showToast(value === 'trash' ? '当前账号不能删除该文件' : '当前文件不能恢复', 'error'); return }
    if (value === 'trash' && loaded.blockers.length > 0) { showToast('文件正在被业务记录引用，暂不能删除', 'error'); return }
    setAction(value); setReason(''); setGrants(loaded.grants.map(item => ({ ...item }))); setRequestId(crypto.randomUUID())
  }
  ...
  return <div className="fde-workspace">
    <Card className="fde-panel overflow-hidden">
      ...
      {error ? <p role="alert" className="p-5 text-sm text-red-600">文件加载失败：{error}</p> : !list ? <p className="p-5 text-sm text-slate-500">正在读取有权访问的文件…</p> : <>
        <div className="fde-detail-file-grid">{list.list.map(file => <article className="fde-detail-file-card" key={file.id}><button onClick={() => open(file.id)}><span className="fde-detail-file-icon">{file.name.split('.').pop()?.slice(0, 4).toUpperCase() || 'FILE'}</span><span><strong>{file.name}</strong><small>{file.category}</small><small>{bytes(file.byteSize)} · {file.uploader}</small></span></button><footer><StatusBadge status={file.parseStatus} /><span className="fde-detail-file-card-actions"><button onClick={() => open(file.id)}>{view === 'deleted' ? '查看详情' : '查看详情'} →</button>{file.capabilities?.trash && <button className="danger" aria-label={`删除文件：${file.name}`} title="移入回收站" onClick={() => { void chooseListAction(file.id, 'trash') }}><Trash2 className="h-4 w-4" />删除</button>}{file.capabilities?.restore && <button aria-label={`恢复文件：${file.name}`} onClick={() => { void chooseListAction(file.id, 'restore') }}><ArchiveRestore className="h-4 w-4" />恢复</button>}</span></footer></article>)}</div>
    ...
    <Modal open={Boolean(action && detail)} title={action === 'permissions' ? '设置文件权限' : action === 'trash' ? '删除项目文件' : '恢复文件'} onClose={() => { if (!busy) setAction(null) }} width="max-w-2xl" footer={<><Button variant="secondary" disabled={busy} onClick={() => setAction(null)}>取消</Button><Button variant={action === 'trash' ? 'danger' : 'primary'} loading={busy} disabled={reason.trim().length < 5} onClick={() => { void submit() }}>确认{action === 'permissions' ? '保存权限' : action === 'trash' ? '删除' : '恢复'}</Button></>}>
