...
export function LeadDetailPage() {
  ...
  const hasCompanyTeamContent = !research && (
    Boolean(verifiedTeamIntroduction || generatedTeamIntroduction || sourceLabeledField('teamIntroduction'))
    || team.length > 0
  )
  ...
  return <div className="lead-review-page">
    <div className="lead-review-shell">
      ...
      <main className="lead-review-content">
        ...
        {companyFundingRounds.length > 0 && <ReviewSection title="融资历史" icon={<Building2 />}><div className="lead-review-funding-table" role="table" aria-label="企业融资历史"><div className="lead-review-funding-head" role="row"><span role="columnheader">轮次</span><span role="columnheader">时间</span><span role="columnheader">金额</span><span role="columnheader">估值</span><span role="columnheader">投资方</span><span role="columnheader">来源</span></div>{companyFundingRounds.map((round, index) => {
        ...
        {hasCompanyTeamContent && <ReviewSection title="核心团队" icon={<UsersRound />}><>{(verifiedTeamIntroduction || generatedTeamIntroduction || sourceLabeledField('teamIntroduction')) && <p className="lead-review-team-summary">{verifiedTeamIntroduction ? displayFactValue(verifiedTeamIntroduction.value) : generatedTeamIntroduction || sourceLabeledNode('teamIntroduction', '')}</p>}{team.length > 0 && <div className="lead-review-team-list">{team.map((member, index) => <TeamMemberCard member={member} index={index} key={`${member.name}-${index}`} />)}</div>}</></ReviewSection>}
