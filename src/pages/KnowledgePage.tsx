    ...
    <div>
      ...
      <div className="grid grid-cols-[270px_minmax(0,1fr)] gap-5">
        <Card className="self-start overflow-hidden">
          <div className="border-b border-slate-100 px-4 py-4"><p className="text-xs font-semibold text-slate-700">项目目录</p><p className="mt-1 text-xs text-slate-400">资料必须归属项目或机构公共库</p></div>
          <div className="p-2">
            <button onClick={() => setSelectedProjectId('all')} className={`mb-1 flex w-full items-center gap-3 rounded-lg px-3 py-2.5 text-sm ${selectedProjectId === 'all' ? 'bg-brand-50 font-medium text-brand-700' : 'text-slate-600 hover:bg-slate-50'}`}><BookOpen className="h-4 w-4" /><span className="flex-1 text-left">全部项目资料</span><span className="text-xs text-slate-400">{files.length}</span></button>
            <p className="border-t border-slate-100 px-3 pb-2 pt-3 text-xs font-medium text-slate-400">我的专属项目</p>
            {accessibleProjects.filter((project) => project.owner === currentUser.name).map((project) => <button key={project.id} onClick={() => setSelectedProjectId(project.id)} className={`mb-1 flex w-full items-center gap-3 rounded-lg px-3 py-2.5 text-sm ${selectedProjectId === project.id ? 'bg-brand-50 font-medium text-brand-700' : 'text-slate-600 hover:bg-slate-50'}`}><FolderOpen className="h-4 w-4 shrink-0" /><span className="min-w-0 flex-1 truncate text-left">{project.name}</span><span className="text-xs text-slate-400">{files.filter((file) => file.projectId === project.id).length}</span></button>)}
            <p className="border-t border-slate-100 px-3 pb-2 pt-3 text-xs font-medium text-slate-400">协作项目</p>
            {accessibleProjects.filter((project) => project.owner !== currentUser.name).map((project) => <button key={project.id} onClick={() => setSelectedProjectId(project.id)} className={`mb-1 flex w-full items-center gap-3 rounded-lg px-3 py-2.5 text-sm ${selectedProjectId === project.id ? 'bg-brand-50 font-medium text-brand-700' : 'text-slate-600 hover:bg-slate-50'}`}><FolderOpen className="h-4 w-4 shrink-0" /><span className="min-w-0 flex-1 truncate text-left">{project.name}</span><span className="text-xs text-slate-400">{files.filter((file) => file.projectId === project.id).length}</span></button>)}
          </div>
        </Card>
        ...
        <div>
          ...
          <Card className="overflow-hidden">
            ...
            {view === 'list' ? <DataTable headers={['资料名称', '所属项目', '分类', '上传人', '权限范围', '解析状态', '更新时间', '操作']}>{filtered.map((file) => <tr key={file.id} className="hover:bg-slate-50"><TableCell><span className="flex items-center gap-3"><span className="grid h-9 w-9 place-items-center rounded-lg bg-blue-50 text-xs font-semibold text-blue-600">{getFileTypeLabel(file)}</span><span><span className="block font-medium text-slate-700">{file.name}</span><span className="mt-1 block text-xs text-slate-400">{file.size} · V{file.version}</span></span></span></TableCell><TableCell>{projects.find((project) => project.id === file.projectId)?.name ?? '机构公共知识'}</TableCell><TableCell><Badge>{file.category}</Badge></TableCell><TableCell>{file.uploader}</TableCell><TableCell>{file.visibility}</TableCell><TableCell><StatusBadge status={file.parseStatus} /></TableCell><TableCell>{file.uploadedAt.slice(5)}</TableCell><TableCell><button onClick={() => handleDeleteFile(file)} className="rounded-md px-2 py-1 text-xs text-rose-600 hover:bg-rose-50">删除</button></TableCell></tr>)}</DataTable> : <div className="grid grid-cols-3 gap-4 p-5">{filtered.map((file) => <div key={file.id} className="rounded-xl border border-slate-200 p-4 hover:border-brand-200"><span className="grid h-10 w-10 place-items-center rounded-lg bg-blue-50 text-xs font-semibold text-blue-600">{getFileTypeLabel(file)}</span><p className="mt-4 truncate text-sm font-medium text-slate-700">{file.name}</p><p className="mt-1 text-xs text-slate-400">{projects.find((project) => project.id === file.projectId)?.name ?? '机构公共知识'} · {file.category}</p><div className="mt-3 flex items-center justify-between"><StatusBadge status={file.parseStatus} /><button onClick={() => handleDeleteFile(file)} className="text-xs text-rose-600 hover:underline">删除</button></div></div>)}</div>}
